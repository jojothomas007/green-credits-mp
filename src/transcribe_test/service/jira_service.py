...
from transcribe_test.dto.jira_comment_dto import JiraCommentsResponse
...
class JiraService:
    ...
    def get_comments(self, issue_id_or_key:str) -> JiraCommentsResponse:
        request_url = f"{self.jira_api_url}/issue/{issue_id_or_key}/comment"
        response = self.request_sender.get_request(request_url, self.headers, self.auth)
        return JiraCommentsResponse.model_validate_json(response.content)